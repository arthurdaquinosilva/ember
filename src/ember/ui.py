...
from prompt_toolkit.filters import Condition, has_completions, has_focus, has_selection
...
from ember.transform import is_complete, next_indent
...
def _history():
    try:
        return FileHistory(str(data_dir() / "history"))
    except OSError:
...
class Repl:
    ...
    def _bindings(self) -> KeyBindings:
        ...
        @kb.add("enter", filter=focused & ~has_selection)
        def _enter(event: KeyPressEvent) -> None:
            ...
            if state:
                b.cancel_completion()
            doc = b.document
            at_end = not doc.text_after_cursor.strip()
            if at_end and is_complete(b.text):
                text = b.text
                if text.strip():
        ...
        @kb.add("escape", "enter", filter=focused)
        @kb.add("c-j", filter=focused)
        def _newline(event: KeyPressEvent) -> None:
            self._newline(event.current_buffer)

        @kb.add("tab", filter=focused & ~has_selection)
        ...
        @kb.add("backspace", filter=focused & Condition(self._in_indent))
